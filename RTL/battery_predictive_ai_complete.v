...
module battery_predictive_ai_complete (
    ...
    // FIX (multiple-driver): loop counter `i` and scratch `curr_sum_tmp` were
    ...
    always @(posedge clk_ai or negedge rst_ai_n) begin : current_monitor
        integer i;
        reg [31:0] curr_sum_tmp;  // temp for current averaging
        if (!rst_ai_n) begin
    ...
    always @(posedge clk_ai or negedge rst_ai_n) begin : predictive
        if (!rst_ai_n) begin
            ...
            if (sensor_valid[8] && sensor_valid[9]) begin
                remaining_capacity <= (soc * soh) / 16'd1000;
            end

            // Calculate internal resistance
            if ((current_avg > 16'd100) && (volt_history[0] > 16'd0)) begin
                if (volt_history[0] > batt_pack_volt) begin
                    internal_resistance <= ((volt_history[0] - batt_pack_volt) * 16'd10) / current_avg;
                end
            end
