...
#
# Notes on this file vs the previous one:
#   1. FIXED: AXI input/output delays separated correctly by port DIRECTION, not prefix.
#      Old code used "s_axi_*" for set_input_delay which captured output ports too,
#      and "m_axi_*" for set_output_delay which captured input ports too.
#   2. FIXED: Input/output delay values corrected to stay within the clock period.
#      Old clk_200mhz_mcu had -max 8.0 on a 5 ns period – impossible.
#   3. FIXED: All -min (hold) values added.  OpenROAD needs both max and min.
#   4. FIXED: Redundant inter-clock set_false_path lines removed.
#      set_clock_groups -asynchronous already implies false paths between every
#      pair of clock groups; duplicating them with set_false_path causes
#      conflicting-constraint warnings in OpenROAD.
#   5. FIXED: Missing ports added – power inputs, pwr_en outputs, sensor_digital_*
#   6. FIXED: sky130 max_transition tightened to 0.4 ns (0.5 is marginal on HD cells).
#   7. ADDED:  Multicycle path comment block explaining why none are needed here.
# =============================================================================
...
# setup uncertainty is conservative and appropriate.  Hold uncertainty is 0.05 ns.
# -----------------------------------------------------------------------------
...
# clk_50mhz_sensor(20ns):in_max=7.0, in_min=0.0, out_max=9.0, out_min=2.0
# clk_200mhz_mcu (5 ns): in_max=1.8, in_min=0.0, out_max=2.2, out_min=0.5
# -----------------------------------------------------------------------------


# ---- 4a. AON domain (clk_10mhz_aon, 100 ns period) – Inputs ----------------
set_input_delay -clock clk_10mhz_aon -max 35.0 -min 0.0 \
    [get_ports {mode_switch_car mode_switch_bike mode_auto_detect \
...
# ai_processing_active: combinational (pwr_en_ai & pll_locked & mode_valid), AON path
# safety_health_status: combinational mux using crash_latched (clk_aon)
set_output_delay -clock clk_10mhz_aon -max 45.0 -min 10.0 \
    [get_ports {pwr_en_ai pwr_en_sensor pwr_en_mcu power_state \
...
set_input_delay -clock clk_50mhz_sensor -max 7.0 -min 0.0 \
    [get_ports {sensor_adc_in_0  sensor_adc_in_1  sensor_adc_in_2  sensor_adc_in_3 \
...
set_input_delay -clock clk_50mhz_sensor -max 7.0 -min 0.0 \
    [get_ports {sensor_adc_valid_0  sensor_adc_valid_1  sensor_adc_valid_2  sensor_adc_valid_3 \
...
set_input_delay -clock clk_50mhz_sensor -max 7.0 -min 0.0 \
    [get_ports {sensor_adc_channel}]

set_input_delay -clock clk_50mhz_sensor -max 7.0 -min 0.0 \
    [get_ports {sensor_digital_in_0  sensor_digital_in_1  sensor_digital_in_2  sensor_digital_in_3 \
...
set_input_delay -clock clk_50mhz_sensor -max 7.0 -min 0.0 \
    [get_ports {sensor_digital_valid_0  sensor_digital_valid_1  sensor_digital_valid_2 \
...
set_output_delay -clock clk_50mhz_sensor -max 9.0 -min 2.0 \
    [get_ports {sensor_enable sensor_grace_active sensor_fault sensor_valid_out}]
...
set_output_delay -clock clk_100mhz -max 4.5 -min 1.0 \
    [get_ports {system_health_score \
                battery_health_status motor_health_status thermal_health_status}]


# ---- 4f. MCU domain (clk_200mhz_mcu, 5 ns period) – Inputs ----------------
# FIX: Only ACTUAL INPUT ports listed here.
# s_axi slave inputs: awvalid, awaddr, awprot, wvalid, wdata, wstrb, bready,
#                     arvalid, araddr, arprot, rready
# m_axi master inputs (from external interconnect): awready, wready,
#                     bvalid, bresp, arready, rvalid, rdata, rresp
set_input_delay -clock clk_200mhz_mcu -max 1.8 -min 0.0 \
    [get_ports {s_axi_awvalid s_axi_awaddr  s_axi_awprot \
                s_axi_wvalid  s_axi_wdata   s_axi_wstrb  s_axi_bready \
                s_axi_arvalid s_axi_araddr  s_axi_arprot s_axi_rready}]

set_input_delay -clock clk_200mhz_mcu -max 1.8 -min 0.0 \
    [get_ports {m_axi_awready m_axi_wready  \
...
# FIX: Only ACTUAL OUTPUT ports listed here.
# s_axi slave outputs: awready, wready, bvalid, bresp, arready, rvalid, rdata, rresp
# m_axi master outputs: awvalid, awaddr, awprot, wvalid, wdata, wstrb, bready,
#                       arvalid, araddr, arprot, rready
# Fault log write bus, debug outputs, test outputs
set_output_delay -clock clk_200mhz_mcu -max 2.2 -min 0.5 \
    [get_ports {s_axi_awready s_axi_wready  s_axi_bvalid  s_axi_bresp \
                s_axi_arready s_axi_rvalid  s_axi_rdata   s_axi_rresp}]

set_output_delay -clock clk_200mhz_mcu -max 2.2 -min 0.5 \
    [get_ports {m_axi_awvalid m_axi_awaddr  m_axi_awprot \
                m_axi_wvalid  m_axi_wdata   m_axi_wstrb   m_axi_bready \
                m_axi_arvalid m_axi_araddr  m_axi_arprot  m_axi_rready}]

set_output_delay -clock clk_200mhz_mcu -max 2.2 -min 0.5 \
    [get_ports {fault_log_wr_en fault_log_addr fault_log_data \
...
#   - The diagnostic_report_generator has an internal report_interval counter
#     (100 cycles between updates) but the outputs at the top level (system_
#     health_score etc.) are registered every cycle – no multicycle SDC needed.
#   - The sensor_grace_manager has a 1,000,000-cycle timeout but this is purely
#     internal; the sensor_grace_active output is combinational every cycle.
#   - CDC paths between clock domains are handled by set_clock_groups -async
#     above (which disables timing analysis across domains), making multicycle
#     declarations on cross-domain paths both unnecessary and incorrect.
#
# If timing fails on a specific internal path after P&R, a targeted multicycle
# constraint can be added at that point, e.g.:
...
# max_transition: sky130_fd_sc_hd typical max is 0.4 ns at tt/1.8V/25°C
